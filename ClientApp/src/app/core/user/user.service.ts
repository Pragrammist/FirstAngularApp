...
export class UserService{
    ...
    //используется для того чтобы
    //вызвать событие логин
    private userLogin$ = new Subject();

    //используется чтобы отреагировать
    //на событие логин через метод subscribe
    private userLoginObs = this.userLogin$.asObservable();
    ...
    subscribeToLogIn(subscribe:()=>void){
        this.userLoginObs.subscribe(subscribe);
    }   
